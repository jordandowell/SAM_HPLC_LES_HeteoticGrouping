...
PCA.IND<-fviz_pca_ind(FlavPhen.PCA,geom.ind = "point",pointshape=19, label=SAMMETA$CORE12,col.ind = SAMMETA$CLASS, mean.point=F)
...
ggplot(
  ...
  guides(size = "none",fill = guide_legend(override.aes=list(shape=21))) + geom_hline(yintercept =0,linetype = "dashed",color = "black") + geom_vline(xintercept = 0,linetype = "dashed",color = "black") + theme_bw()
ggplot(
  cbind(PCA.IND$data, SAMMETA),
  aes(
    x = x,
...
for (i in 2:20) {
   print(ggviolin(y=colnames(Combodataframe)[i],x= "BREED",add = c("jitter", error.plot = "crossbar"), data = Combodataframe, draw_quantiles = c(0.025,0.5,0.975)))
 }
dev.off()
...
for (i in 2:20) {
trait.range<-range(Combodataframe[,i])[2]-range(Combodataframe[,i])[1]

Trait<-colnames(Combodataframe)[i]
#bftest has a error but we need the loop to continue 
bf <- ttestBF(formula = reformulate(termlabels="BREED",response = colnames(Combodataframe)[i] ), data = Combodataframe)
bf.frame<-data.frame(bf)
chains <- posterior(bf, iterations = 100000)
chains.frame<-data.frame(unclass(summary(chains)))
#save results 
results<-data.frame()
results<-cbind(bf.frame$bf,bf.frame$error, (chains.frame$statistics.Mean[2]/abs(trait.range)),(chains.frame$statistics.SD[2]/abs(trait.range)),(chains.frame$quantiles.2.5.[2]/abs(trait.range)),(chains.frame$quantiles.97.5.[2]/abs(trait.range)))
row.names(results)<-Trait
colnames(results)<-c("BF","BFerror","mean","mean.SD","quant.2.5","quant.97.5")
...
for (i in 2:20) {
  print(ggviolin(y=colnames(Combodataframe)[i],x= "USE",add = c("jitter", error.plot = "crossbar"), data = Combodataframe, draw_quantiles = c(0.025,0.5,0.975)))
}
dev.off()
...
for (i in 2:20) {
  ...
  Trait<-colnames(Combodataframe)[i]
  #bftest has a error but we need the loop to continue 
  bf <- ttestBF(formula = reformulate(termlabels="USE",response = colnames(Combodataframe)[i] ), data = Combodataframe)
  bf.frame<-data.frame(bf)
  chains <- posterior(bf, iterations = 100000)
  chains.frame<-data.frame(unclass(summary(chains)))
  #save results 
...
PCA.IND <-
  fviz_pca_ind(LES_Traits.PCA,geom = "point",axes = c(1,2), label="none", habillage= SAMMETA$CLASS, mean.point=F)
...
ggviolin(y="x",ylab="PC1",x= "BREED",add = c("jitter", error.plot = "crossbar"), data = Combodataframe, draw_quantiles = c(0.025,0.5,0.975))
ggviolin(y="y",ylab="PC2",x= "BREED",add = c("jitter", error.plot = "crossbar"), data = Combodataframe, draw_quantiles = c(0.025,0.5,0.975))
dev.off()
...
ggviolin(y="x",ylab="PC1",x= "USE",add = c("jitter", error.plot = "crossbar"), data = Combodataframe, draw_quantiles = c(0.025,0.5,0.975))
ggviolin(y="y",ylab="PC2",x= "USE",add = c("jitter", error.plot = "crossbar"), data = Combodataframe, draw_quantiles = c(0.025,0.5,0.975))
dev.off()
#create a for loop for results of LES by breed 
...
PCA.IND <-
  fviz_pca_ind(
    LES_Traits.PCA,geom = "point",axes = c(1,2), label="none", habillage= SAMMETA$CLASS, mean.point=F)
...
Ecophysummary<-describeBy(Combodataframe[,c(4,9:ncol(Combodataframe))], group = 'BREED',mat=TRUE)

write.csv(Ecophysummary,"OUTPUT/EcophysSummarySTATS_BREED.csv")

Ecophysummary<-describeBy(Combodataframe[,c(7,9:ncol(Combodataframe))], group = 'USE',mat=TRUE)
...
colnames(PeakAreas200)<- paste0("PeakAreas_RT",colnames(PeakAreas200))
colnames(RelativeRatios)<- paste0("PeakAreas_RT",colnames(RelativeRatios))
colnames(RelativeRatios200)<- paste0("PeakAreas_RT",colnames(RelativeRatios200))
#cbind because rows are same order 
Combodataframe<-cbind.data.frame(PeakAreas,PeakAreas200,RelativeRatios,RelativeRatios200)
View(Combodataframe)
#create a summary stats table of data 
...
PCA.IND<-fviz_pca_ind(PeakAreas.PCA,geom = "point",axes = c(1,2), label="none", habillage= SAMMETA$CLASS, mean.point=F)
...
PCA.IND<-fviz_pca_ind(PeakAreas200.PCA,geom = "point",axes = c(1,2), label="none", habillage= SAMMETA$CLASS, mean.point=F)
...
write.csv(ECOPHYS_results,"OUTPUT/PeakAreas200_bayesFactor_BREED.csv")
...
PCA.IND<-fviz_pca_ind(RelativeRatios.PCA,geom = "point",axes = c(1,2), label="none", habillage= SAMMETA$CLASS, mean.point=F)
...
ggviolin(y="x",ylab="PC1",x= "BREED",add = c("jitter", error.plot = "crossbar"), data = Combodataframe, draw_quantiles = c(0.025,0.5,0.975))
ggviolin(y="y",ylab="PC1",x= "BREED",add = c("jitter", error.plot = "crossbar"), data = Combodataframe, draw_quantiles = c(0.025,0.5,0.975))
dev.off()
...
write.csv(ECOPHYS_results,"OUTPUT/RelativeRatios_bayesFactor_BREED.csv")
...
write.csv(ECOPHYS_results,"OUTPUT/RelativeRatios_bayesFactor_USE.csv")
...
PCA.IND<-fviz_pca_ind(RelativeRatios200.PCA,geom = "point",axes = c(1,2), label="none", habillage = SAMMETA$CLASS, mean.point=F)
  Combodataframe<-cbind(PCA.IND$data, SAMMETA)
#Bayes factor of each group
...
write.csv(ECOPHYS_results,"OUTPUT/RelativeRatios200_bayesFactor_BREED.csv")
...
write.csv(ECOPHYS_results,"OUTPUT/RelativeRatios200_bayesFactor_USE.csv")
...
write.csv(Ecophysummary,"OUTPUT/HPLC_composites_SummarySTATS_BREED.csv")
